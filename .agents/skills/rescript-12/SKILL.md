...
Add `<link rel="stylesheet" href="/src/index.css" />` in `index.html`. Do NOT rely on Vite auto-discovery alone.

### System-preference dark mode (no JS needed)

Use `@media (prefers-color-scheme: dark) { :root { ... } }` instead of `.dark { }`. Remove `class="dark"` from `<html>`. Tailwind v4's `dark:` variants and your CSS variable overrides will both fire at the same media query.
